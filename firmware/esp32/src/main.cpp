...
double steering = 0.0;      // Referencia de posición (°) por serial (double soporta decimales)
double pwm = 0;
int pwm0 = 0;
...
static bool readNumberFromSerial(double &outValue) {
  if (imu_x_deg > 179 && imu_x_deg < 181){
      double v = 0.0;
      outValue = v;
      return true;
  }
...
void loop(void) 
{
  ...
  if (readNumberFromSerial(val)) {
    ...
    Serial.print(">ref:"); Serial.println(steering, 2);
  }
