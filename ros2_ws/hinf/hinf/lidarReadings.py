...
class LidarLeftWithIMU(Node):
    def __init__(self):
        ...
        self.declare_parameter('max_proj', 1.5)         # m (umbral proyección verde)
        # ───────────────────────────────────────────────────────────────────────

        # === NUEVO: parámetros del CONO ESTRECHO (rojo) ===
        self.declare_parameter('narrow_aperture_deg', 10.0)   # ancho total (deg)
        self.declare_parameter('narrow_max_proj', 1.5)      # m (umbral proyección estrecho)
        # === NUEVO: parámetros de CONTEO NARROW ===
        self.declare_parameter('narrow_min_valids', 3)       # mínimo de lecturas válidas para contar
        self.declare_parameter('narrow_refractory_sec', 24.0) # tiempo mínimo entre conteos
        ...
        self.eYaw = 0.0
        # en __init__
        self.imu_target_rad = 0.0  # por defecto: 0°
        self.create_subscription(Float32, 'imu_target_deg', self.cb_imu_target, 10)
        ...
        self.pub_Postes = self.create_publisher(Float32, 'Postes', 10)
    ...
    def cb_imu_target(self, msg: Float32):
        self.imu_target_rad = math.radians(float(msg.data))  # 0 o 180 típicamente
        self.get_logger().info(f'🎯 IMU target actualizado a {float(msg.data):.1f}°')
    ...
    def callback_imu(self, msg: Float32):
        ...
        self.eYaw = wrap_to_pi(self.imu_target_rad - self.imu_angle)
    ...
    def process_scan(self):
        try:
            ...
            if count_nar > 0:
                ...
                self.get_logger().info(f"🔴 NARROW válidas=0, Postes={self.narrow_count_total}, Eyaw: {self.eYaw}")
            Postes = float(self.narrow_count_total)
            self.pub_Postes.publish(Float32(data=Postes))  

            # === CONTEO NARROW con refractario (nuevo) ===
            if self.eYaw>2.9 or self.eYaw<-2.9: 
                self.narrow_count_total=0
